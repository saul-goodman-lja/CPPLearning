...
{    
    ...
    off_t offset_bytes;
    int store_fd ;
    store_fd = open(store_name, O_RDWR);
    if(store_fd == -1){
        off_t x = 0;
        store_fd = open(store_name, O_RDWR | O_CREAT, 0666);
        write(store_fd, &x, sizeof(off_t));
        ftruncate(file_fd, file_size);
    }

    lseek(store_fd, 0, SEEK_SET);
    read(store_fd, &offset_bytes, sizeof(off_t));
    ...
    {   
        lseek(store_fd, 0, SEEK_SET);
        write(store_fd, &offset_bytes, sizeof(off_t));
